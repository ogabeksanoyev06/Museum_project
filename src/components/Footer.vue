<template>
  <div class="footer">
    <div class="container footer_menu">
      ...
      <ul class="d-md-flex footer_items">
        <li
          class="me-0 me-md-3"
          ...
          :key="index"
        >
          <router-link @click="$route.name === item.name" :to="{ name: item.name }">{{
            item.title
          }}</router-link>
        </li>
      </ul>
      <ul class="d-flex align-items-center footer_t">
        <li>
...
.footer_items li a:hover::before {
  position: absolute;
  content: "";
  left: 50%;
  bottom: -10px;
